...
from chip_pattern import generate_reference_and_background_maps
...
def generate_video_and_masks(params, trajectories, ipsf_interpolators):
    ...
    In the current implementation these maps are spatially uniform, so the
    behavior matches the original scalar-based code exactly. This abstraction
    allows chip/substrate patterns to be introduced later without changing the
    overall rendering structure.
    """
    ...
    # --- Stationary reference field and background maps ---
    # The reference field and background intensity are modeled as 2D maps so
    # that chip/substrate patterns can be introduced via this interface while
    # keeping the rest of the rendering loop unchanged. For now these maps are
    # spatially uniform, matching the original scalar behavior.
    fov_shape_os = (os_size, os_size)
    E_ref_os, E_ref_final, background_final = generate_reference_and_background_maps(
        params,
        fov_shape_os=fov_shape_os,
        final_fov_shape=final_size,
    )
    E_ref_intensity_os = np.abs(E_ref_os) ** 2
    E_ref_intensity_final = np.abs(E_ref_final) ** 2
